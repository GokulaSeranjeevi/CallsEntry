...
import com.jilaba.calls.query.DevCallsQuery;
import com.jilaba.common.ReturnStatus;
import com.jilaba.exception.ErrorHandling;
import com.jilaba.exception.JilabaException;

@Component
public class DevCallsDaoImpl implements DevCalls {
	...
	@Autowired
	private DevCallsQuery devCallsQuery;
	...
	public ReturnStatus getDeveloper() {
		...
		try {
			...
			lstDevelopers = jdbcTemplate.query(devCallsQuery.getDevelopers(), new DeveloperRowMapper());
	...
	public ReturnStatus getDepartment() {
		...
		try {
			...
			lstDept = jdbcTemplate.query(devCallsQuery.getDepartment(), new DeptRowMapper());
	...
	public ReturnStatus getModule(Integer deptNo) {
		...
		try {
			...
			lstModules = jdbcTemplate.query(devCallsQuery.getModules(deptNo), new ModuleRowMapper());
	...
	public ReturnStatus getCustomer() {
		...
		try {
			...
			lstCustomer = jdbcTemplate.query(devCallsQuery.getCustomer(), new CustomerRowMapper());
	...
	public ReturnStatus getCustCoOrd() {
		...
		try {
			...
			lstDeptAuthorize = jdbcTemplate.query(devCallsQuery.getCustCoOrd(), new DeptAuthorizeRowMapper());
	...
	public ReturnStatus getRecvFrom() {
		...
		try {
			...
			lstRecvFrom = jdbcTemplate.query(devCallsQuery.getRecvFrom(), new RecvFromRowMapper());
	...
	public ReturnStatus getCallCoOrd() {
		...
		try {
			...
			lstCallCoOrd = jdbcTemplate.query(devCallsQuery.getCallCoOrd(), new CallCoOrdRowMapper());
	...
	public ReturnStatus getDeveloperCalls(String asOnDate, long strCallNo, int strDeveleoper, int strDepartment,
		...
		try {
			...
			// Construct query
			String sqlQuery = devCallsQuery.getDeveloperCalls(asOnDate, strDeveleoper, strDepartment, strModule,
...
//			e.printStackTrace();
	...
	public ReturnStatus getCallImages(String callno) {
		...
		try {
			...
			lstCallsImages = jdbcTemplate.queryForObject(devCallsQuery.getCallsImages(callno),
	...
	public ReturnStatus updateProgressCall(int callNo) {
		...
		try {
			...
			jdbcTemplate.update(devCallsQuery.updateProgressCall(), new Object[] { callNo });
	...
	public ReturnStatus validateProgressCall(int Developer) {
		...
		List<Calls> lstCall;
		try {
			lstCall = jdbcTemplate.query(devCallsQuery.validateProgressCall(Developer), new validateProgressCall());
	...
	public ReturnStatus updateReadyCalls(int callNo, String readyDesc) {
		try {
			...
			jdbcTemplate.update(devCallsQuery.updateReadyCals(), new Object[] { readyDesc, callNo });
			return new ReturnStatus(true);
	...
	public ReturnStatus getReturnCall(int Developer) {
		try {
			...
			lstReturnCalls = jdbcTemplate.query(devCallsQuery.getReturnCall(Developer), new validateReturnCall());
	...
	public ReturnStatus validateReadyCalls(String callNo) {
		List<Calls> lstCall;
		try {
			lstCall = jdbcTemplate.query(devCallsQuery.validateReadyCalls(callNo), new validateReadyCalls());
	...
	public ReturnStatus insertProgressCall(String callNo) {
		...
		try {
			...
			jdbcTemplate.update(devCallsQuery.insertProgressCall(callNo), new Object[] {});
			return new ReturnStatus(true);
		} catch (Exception e) {
	...
	public ReturnStatus updateProgressCancel(String callNo) {
		...
		try {
			...
			jdbcTemplate.update(devCallsQuery.updateProgressCancel(callNo), new Object[] {});
