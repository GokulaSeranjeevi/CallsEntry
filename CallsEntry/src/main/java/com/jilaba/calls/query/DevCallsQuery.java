...
public class DevCallsQuery {
	...
	public String getDeveloperCalls(String asOnDate, int devCount, int deptCount, int modIdCount, int custCount,
		...
		sb.append(
				...
				.append("WHERE C.cdate <='" + asOnDate + "' And C.sugto<>0  And S.designation=1 \r\n");
	...
	public String updateReadyCals() {
		...
		sb.append("Update Calls Set Ready='Y'," + " ready_date='" + formatter.format(now) + "',ready_desc= ?"
				+ ",ready_time= '" + formatter.format(now) + "' Where callno=? ");
