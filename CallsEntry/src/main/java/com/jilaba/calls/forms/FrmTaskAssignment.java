...
@Scope("prototype")
public class FrmTaskAssignment extends JFrame implements ActionListener, KeyListener {
	...
	private JilabaComboBox<Operator> cmbStaff;
	...
	private List<Operator> lstStaff;
	...
	private Color color1 = Color.decode("#a63a50");
	private Color color2 = Color.decode("#FFFFFF");
	private Color color3 = Color.decode("#c07584");
	private Color color4 = Color.decode("#AED6F1");
	private Color color5 = Color.decode("#e4dedf");
	private Color color6 = Color.decode("#edd7dc");
	...
	@Autowired
	private LogicTaskAssignment logicTask;
	@Autowired
	...
	private void createKeyListeners() {
		...
		txtDevPriority.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					logicTask.updateDevPriority(txtDevPriority.getText(), SelectedCallno);
		...
		tblDevCalls.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_SPACE) {
					if (FrmLogin.designation == 3 && !cmbStaff.getSelectedItem().equals("All")) {
						enableTextField();
				...
				if (e.getKeyCode() == KeyEvent.VK_P) {
					...
					if (FrmLogin.OperCode == Integer.valueOf(String.valueOf(cmbStaff.getSelectedItemValue()))) {

						lstList = new ArrayList<Calls>();

						lstList = logicTask.validateReadyCalls(
						...
						if (!lstList.get(0).getReady().equalsIgnoreCase("Y")) {
							...
							if (response == JOptionPane.YES_OPTION) {
								...
								if ((progress.size() == 0 && strType != 2) && (progress.size() == 0 && strType != 3)) {
									...
									logicTask.updateProgressCall(callNo);
				...
				if (e.getKeyCode() == KeyEvent.VK_R) {
					...
					String moduleid = String
							.valueOf(tblDevCalls.getModel().getValueAt(tblDevCalls.getSelectedRow(), 8));
					...
					if (FrmLogin.OperCode == Integer.valueOf(String.valueOf(cmbStaff.getSelectedItemValue()))) {
						...
						if (!lstList.get(0).getReady().equalsIgnoreCase("Y")) {
							...
							if (response == JOptionPane.YES_OPTION) {
								...
								logicTask.insertProgressCall(String
										.valueOf(tblDevCalls.getModel().getValueAt(tblDevCalls.getSelectedRow(), 0)));

								logicTask.updateProgressCancel(String
										.valueOf(tblDevCalls.getModel().getValueAt(tblDevCalls.getSelectedRow(), 0)));
							}
		...
		cmbModule.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lstModule = logicTask.getModule(Integer.valueOf(String.valueOf(cmbDepartment.getSelectedItemValue())));
	...
	private ReturnStatus loadInitialize() {
		...
		cmbStaff.addListItem(new ListItem("All", 0));
		...
		lstStaff = logicTask.getDeveloper();

		for (Operator dev : lstStaff) {
			cmbStaff.addListItem(new ListItem(dev.getStaffname(), dev.getStaffid()));
		}

		lstDepartment = logicTask.getDepartment();

		for (Department dept : lstDepartment) {
			cmbDepartment.addListItem(new ListItem(dept.getDepartment(), dept.getdNo()));
		}

		lstModule = logicTask.getModule(0);

		for (com.jilaba.calls.model.Module module : lstModule) {
			cmbModule.addListItem(new ListItem(module.getModuleName(), module.getModuleId()));
		}

		lstCustomer = logicTask.getCustomer();

		for (Customer customer : lstCustomer) {
			cmbCustomer.addListItem(new ListItem(customer.getcustName(), customer.getCustId()));
		}

		lstCustCoOrd = logicTask.getDeptAuthorize();
		...
		cmbStaff.setSelectedItemValue(FrmLogin.OperCode);
		...
		// lstCheck= logicTask.validateProgressCall();
		//
		// if (lstCheck != null) {
		//
		//// logicTask.updateProgressCall(callNo);
	...
	private Component panelDetail2Initialize() {
		...
		lblStaff = new JLabel("Staff Name");
		lblStaff.setBounds(lblCallNo.getX(), lblCallNo.getY() + lblCallNo.getHeight(), lblWidth, lblHeight);
		lblStaff.setBackground(color2);
		lblStaff.setFont(CustomFonts.fontCalibriBold);
		lblStaff.setVisible(true);
		lblStaff.addKeyListener(this);
		...
		cmbStaff = new JilabaComboBox<Operator>();
		cmbStaff.setBounds(txtCallNo.getX(), lblStaff.getY(), txtWidth, txtHeight);
		...
		lblCustomer.setBounds(lblModule.getX(), cmbStaff.getY(), lblWidth, lblHeight);
	...
	private Component panelDetailInitialize() {
		...
		lblHeading = new JLabel("TASK ASSIGNMENT");
	...
	private void btnView() {
		...
		lstDevCalls = logicTask.getDeveloperCalls(spnAsOnDate.getDateValue(), strCallNo, strDeveleoper, strDepartment,
				strModule, strCustomer, strType, strDeptAuthorize, strRecvFrom, strCallCoOrd, strCallNature);
		...
		progress = logicTask.validateProgressCall(strDeveleoper);

		List<ReturnCalls> returnCalls = new ArrayList<ReturnCalls>();

		returnCalls = logicTask.getReturnCall(strDeveleoper);
	...
	private void btnImageClick() {
		...
		CallsImages callsImages = logicTask.getImages(String.valueOf(lblCallAndTicketval.getText()));
	...
	private ReturnStatus chkSelectAll() {
		...
		if (cmbStaff.getSelectedItem().equals("All")) {
			strDeveleoper = 0;
		} else {
			strDeveleoper = Integer.valueOf(String.valueOf(cmbStaff.getSelectedItemValue()));
		}
